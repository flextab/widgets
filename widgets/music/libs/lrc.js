export class LrcParser {
    ...
    select(ts) {
        ...
        var i = 0;
        if (!this.lyrics_all || !this.lyrics_all.length || timestamp < this.lyrics_all[0].timestamp) {
            return -1;
        }
        for (i = 0; i < this.lyrics_all.length - 1; i++) {
            if (this.lyrics_all[i].timestamp <= timestamp && this.lyrics_all[i + 1].timestamp > timestamp) {
                break;
            }
        }
        return i;
    ...
    get(ts) {
        let id = this.select(ts);
        return (this.getLyric(id) || {}).text || "";
    }
}
